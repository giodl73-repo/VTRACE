...
| VAL-016 | Product adopter / agent operator | Use VTRACE to drive product implementation without turning VTRACE work packages, proof, readiness, evidence, validation, or fixtures into product UX. | VTRACE artifacts separate product requirement, implementation area, verification command, and VTRACE-only closeout; generated briefs forbid product process commands unless requirements explicitly define them as customer-facing behavior. | `README.md`, `vtrace-process.md`, adoption templates, skills, `EVID-062` | passed |
